...
export function buildSystemPrompt(
  ...
  return [
    ...
    `- Rechazar el match actual (usa la herramienta reject_match).`,
    ``,
    ...
    `- Rechazar: solo llama reject_match cuando el usuario diga con claridad que NO`,
    `  quiere este match (ej. "no me interesa", "rechazar"). Confirma su intención en`,
    `  una frase corta antes. Si devuelve "REJECTED", avísale con amabilidad que el`,
    `  match no continúa y que le buscaremos otro en el próximo ciclo.`,
